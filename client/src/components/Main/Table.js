  ...
  return(
    <div className = 'table-container'>
      <div className="table-row">
        <div><a href={ samePapers[0].url } target="_blank">Click here to view paper</a></div>
      </div>
      <div className="table-row">
...
const Table = ({data, search}) => {
  ...
  //console.log('Table state:', tooltipState);

  const value = info?.label || search.value
  return (
    <>
     { show || (!search.isOpen && search.results && search.results.length > 0) ? <p style={{fontWeight: 900}}>Once clicking on a node for more information, unselect that node by clicking the same node to return to the overall view</p>: <p></p> }
     <div className='Table'>
      { show || (!search.isOpen && search.results && search.results.length > 0) ? tableActive(data, value) : tableEntityDefault() }
    </div>
    </>
  )
